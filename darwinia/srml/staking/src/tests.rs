...
fn test_validate() {
	with_externalities(&mut ExtBuilder::default()
		...
		// initial rewards
		assert_eq!(Staking::current_session_reward(), 1000000);
...
fn session_era_epoch_should_work_well() {
	with_externalities(&mut ExtBuilder::default()
		...
		// add 1 validator
		Staking::set_validator_count(3);
		Staking::validate(Origin::signed(3), ValidatorPrefs{ unstake_threshold: 3, validator_payment: 100});
		...
		assert_eq!(Session::validators(), vec![3, 2, 1]);
		// ring total issuance /( 5 * 2)
		assert_eq!(Staking::ideal_era_total_reward(), 998999226600000000);
	});
}

#[test]
fn nominate_and_reward_should_work() {
	with_externalities(&mut ExtBuilder::default()
		.existential_deposit(0).build(), || {
		// 101 - 1
		// 102 - 2
		...
		Kton::deposit(Origin::signed(41), 1000000, 12);
		assert_eq!(Kton::free_balance(&41_u64), 100);
		...
		Staking::bond(Origin::signed(41), 40, 60, RewardDestination::Controller);
		assert_eq!(Staking::bonded(&41), Some(40));

		// nominate stash
		Staking::nominate(Origin::signed(40), vec![102, 101, 103]);
		...
		assert_eq!(Staking::current_elected(), vec![103, 102, 101]);
		assert_eq!(Staking::current_epoch(), 0);
		assert_eq!(Staking::slot_stake(), 105);

		// block 12
		System::set_block_number(12);
		Session::check_rotate_session(System::block_number());
//		assert_eq!(Staking::ideal_era_total_reward(), 12);
		assert_eq!(Kton::bill_lock_rate(), Perbill::from_percent(90));
		assert_eq!(Ring::bill_lock_rate(), Perbill::zero());
