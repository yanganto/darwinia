...
decl_storage! {
	trait Store for Module<T: Trait> as Staking {
		...
		pub IdealEraTotalReward get(ideal_era_total_reward): RewardBalanceOf<T>;
...
impl<T: Trait> Module<T> {
	...
	fn new_era() {
		...
		<CurrentSessionReward<T>>::put(Self::ideal_era_total_reward() / <RewardBalanceOf<T>>::sa(validator_count * sessions_per_era));
	...
	fn new_epoch() {
		...
		if let Ok(next_era_reward) =  minting::compute_next_era_reward::<T>() {
			<IdealEraTotalReward<T>>::put(next_era_reward);
		}
	}
